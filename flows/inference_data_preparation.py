...
boto3_config = Config(
    region_name=AWS_REGION, s3={'addressing_style': 'virtual'}
)
...
def fetch_epa_aqs_data_for_inference(
    ...
    try:
        ...
        if isinstance(data, list):
            if not data:
                logger.info("API response for %s is an empty list, no data.",
                          target_year)
            else:
                df = pd.DataFrame(data)
                logger.info("Successfully fetched %s records for %s (list response).",
                          len(df), target_year)
                all_data_frames.append(df)

        elif isinstance(data, dict):
            header = data.get("Header")
            if (header and isinstance(header, list) and
                header[0].get("status") == "No data meets your criteria"):
                logger.info("No data found for %s.", target_year)
            elif "Data" in data and isinstance(data["Data"], list):
                df = pd.DataFrame(data["Data"])
                logger.info("Successfully fetched %s records for %s (dict response).",
                          len(df), target_year)
                all_data_frames.append(df)
            else:
                logger.warning(
            ...
            logger.warning(
                ...
                "got %s: %s", target_year, type(data), data
            )
    ...
    logger.info("Columns in raw DataFrame for inference: %s",
                combined_df.columns.tolist())
    logger.info("First 5 rows of combined raw data for inference:\n%s",
...
def transform_data_for_inference(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info("Starting data transformation for inference on %s records.",
                len(df))
    ...
    if 'date_local' in df.columns:
        ...
        if len(df) < initial_len:
            logger.info("Dropped %s rows with invalid 'date_local'.",
                       initial_len - len(df))
    else:
    ...
    for col in required_cols_check:
        if col not in df.columns or df[col].isnull().all():
            logger.error("Critical column '%s' is missing or entirely NaN. "
                        "Cannot proceed with transformation.", col)
    ...
    if dropped_rows > 0:
        logger.info("Dropped %s rows with missing critical feature values "
                   "(latitude/longitude).", dropped_rows)

    if df.empty:
        logger.warning("DataFrame became empty after dropping NaNs for essential "
    ...
    columns_for_output = [
        'date_local', 'latitude', 'longitude', 'cbsa_code',
        'state', 'county', 'city',  # Identifiers
        'year', 'month', 'day_of_week', 'day_of_year', 'is_weekend'  # Features
    ]
    ...
    logger.info("Selected %s columns for processed inference data output.",
                len(df.columns))

    logger.info("Data transformation for inference complete. Final shape: %s",
                df.shape)
...
def write_processed_inference_data_to_s3(
    ...
    if df.empty:
        logger.warning("Processed inference DataFrame is empty, "
                      "skipping export to S3.")
        return

    full_s3_key = f"{key_prefix}/{file_name}.parquet"
    logger.info("Uploading processed inference data to s3://%s/%s",
    ...
    try:
        ...
        logger.info("Successfully wrote processed inference data to s3://%s/%s",
                    bucket_name, full_s3_key)
    except Exception as e:
...
def inference_data_preparation_flow(target_year: int = None):
    """
    ...
    if target_year is None:
        ...
        logger.info("No target_year specified. Defaulting to current year: %s",
                    target_year)
    else:
    ...
    processed_df_for_inference = transform_data_for_inference(
        df=raw_df_for_inference
    )

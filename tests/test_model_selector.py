...
class TestGetBestNonOverfitRun:
    ...
    def test_get_best_non_overfit_run_success(self, mock_client_class, mock_experiment, mock_valid_runs):
...
class TestUploadFilesToS3:
    ...
    def test_upload_files_to_s3_success(self, temp_model_dir):
        ...
        response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=f"{prefix}/{run_id}/")
...
class TestCreateAndUploadModelSummary:
    ...
    def test_create_and_upload_model_summary_success(self, mock_run, temp_model_dir):
        ...
        result = create_and_upload_model_summary(mock_run, temp_model_dir, bucket_name, prefix)
...
class TestUploadModelAndSummaryToS3:
    ...
    @patch('flows.model_selector.download_model_artifacts')
    def test_upload_model_and_summary_to_s3_download_failure(self, mock_download, mock_run):
...
class TestModelSelectionFlow:
    ...
    def test_model_selection_flow_success(self, mock_get_best, mock_upload, mock_best_run):
    ...
    @patch('flows.model_selector.upload_model_and_summary_to_s3')
    @patch('flows.model_selector.get_best_non_overfit_run')
    def test_model_selection_flow_upload_failure(self, mock_get_best, mock_upload, mock_best_run):
...
@pytest.mark.parametrize("val_rmse,test_rmse,expected_valid", [
...
def test_run_validation_logic(val_rmse, test_rmse, expected_valid):
    ...
    is_valid = (run.data.metrics.get("val_rmse", 0) > 1e-4 and
                "test_rmse" in run.data.metrics)
    
    assert is_valid == expected_valid


@pytest.mark.parametrize("metrics,expected_best_index", [
    ([{"test_rmse": 0.5}, {"test_rmse": 0.3}, {"test_rmse": 0.7}], 1),  # Second run is best
    ([{"test_rmse": 0.8}, {"test_rmse": 0.6}, {"test_rmse": 0.4}], 2),  # Third run is best
...
def test_best_run_selection_logic(metrics, expected_best_index):
    ...
    sorted_runs = sorted(valid_runs, key=lambda r: r.data.metrics.get("test_rmse", float('inf')))

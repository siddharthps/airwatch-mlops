...
class TestWriteProcessedInferenceDataToS3:
    ...
    @mock_aws
    def test_write_processed_inference_data_to_s3_success(self, processed_inference_data):
    ...
    @mock_aws
    def test_write_processed_inference_data_to_s3_upload_error(self, processed_inference_data):
        """Test handling of S3 upload errors."""
        # Don't create the bucket to simulate an error
        with pytest.raises(Exception):
            write_processed_inference_data_to_s3(
                processed_inference_data, "nonexistent-bucket", "test_prefix", "test_file"
            )
...
@pytest.mark.parametrize("target_year,expected_year", [
    (2024, 2024),
    (2025, 2025),
    (2026, 2026),
])

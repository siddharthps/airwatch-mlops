...
class TestLoadHistoricalDataFromS3:
    ...
    def test_load_historical_data_from_s3_file_not_found(self):
        ...
        with pytest.raises(Exception):
            load_historical_data_from_s3(bucket_name, "processed_data", "nonexistent.parquet")
...
class TestLoadPredictionsFromS3:
    ...
    def test_load_predictions_from_s3_success(self, sample_predictions_data):
        ...
        file_key = f"{key_prefix}/pm25_predictions_{target_year}_20250101_120000.parquet"
        buffer = io.BytesIO()
    ...
    def test_load_predictions_from_s3_multiple_files(self, sample_predictions_data):
        ...
        older_file = f"{key_prefix}/pm25_predictions_{target_year}_20250101_100000.parquet"
        newer_file = f"{key_prefix}/pm25_predictions_{target_year}_20250101_120000.parquet"
...
class TestCreateDataDriftReport:
    ...
    def test_create_data_drift_report_success(
        ...
        target_year = 2025
        report, report_path = create_data_drift_report(reference_data, current_data, target_year)
...
class TestCreateRegressionPerformanceReport:
    ...
    def test_create_regression_performance_report_success(
        ...
        target_year = 2025
        report, report_path = create_regression_performance_report(performance_data, target_year)
        ...
        # Verify report path
        expected_path = "monitoring_reports/regression_performance_report_2025_20250101_120000.html"
        assert report_path == expected_path
        assert report == mock_report
...
class TestModelMonitoringFlow:
    ...
    def test_model_monitoring_flow_success_no_drift(
        self, mock_load_historical, mock_load_predictions, mock_create_drift,
        mock_create_regression, mock_save_report, mock_check_drift,
        sample_reference_data, sample_current_data
    ):
        ...
        mock_create_regression.return_value = (mock_regression_report, "regression_report.html")
...
@pytest.mark.parametrize("drift_result,expected_exception", [
    (False, None),
    (True, ValueError),
])
def test_model_monitoring_flow_drift_scenarios(drift_result, expected_exception):
    ...
    with patch('flows.model_monitoring.load_historical_data_from_s3') as mock_load_hist, \
         ...
         patch('flows.model_monitoring.create_regression_performance_report') as mock_create_reg, \
         patch('flows.model_monitoring.save_evidently_report_to_s3') as mock_save, \

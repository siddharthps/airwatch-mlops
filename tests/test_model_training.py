...
class TestLoadProcessedDataFromS3:
    ...
    def sample_processed_data(self):
        ...
        return pd.DataFrame({
            'date_local': pd.to_datetime(['2023-01-01', '2023-01-02', '2023-01-03']),
...
class TestSplitData:
    ...
    def training_data_complete(self):
        ...
        return pd.DataFrame({
            ...
            'arithmetic_mean': [12.5, 15.2, 10.8, 13.1, 14.7, 11.3, 16.2, 12.9, 15.8, 13.5],
            'latitude': [41.8781] * 10,
            ...
            'first_max_value': [18.0, 20.5, 14.2, 17.1, 19.3, 15.8, 21.2, 16.9, 20.1, 17.5],
    ...
    def training_data_missing_features(self):
        ...
        return pd.DataFrame({
            'date_local': pd.to_datetime(['2023-01-01', '2023-01-02', '2023-01-03', '2023-01-04', '2023-01-05']),
...
class TestTrainAndLogModel:
    ...
    def test_train_and_log_model_success(self, mock_mlflow, sample_training_data):
        ...
        mock_mlflow.sklearn.log_model.assert_called_once_with(model, artifact_path="model")
    ...
    @patch('flows.model_training.mlflow')
    def test_train_and_log_model_experiment_creation(self, mock_mlflow, sample_training_data):
    ...
    @patch('flows.model_training.mlflow')
    def test_train_and_log_model_existing_experiment(self, mock_mlflow, sample_training_data):
...
class TestTrainModels:
    ...
    def test_train_models_success(
        ...
        x_train = sample_complete_data[['latitude', 'longitude', 'year', 'month', 
                                      'day_of_week', 'day_of_year', 'is_weekend']].iloc[:6]
        y_train = sample_complete_data['arithmetic_mean'].iloc[:6]
        x_val = sample_complete_data[['latitude', 'longitude', 'year', 'month', 
                                    'day_of_week', 'day_of_year', 'is_weekend']].iloc[6:8]
        y_val = sample_complete_data['arithmetic_mean'].iloc[6:8]
        x_test = sample_complete_data[['latitude', 'longitude', 'year', 'month', 
                                     'day_of_week', 'day_of_year', 'is_weekend']].iloc[8:]
        ...
        mock_train_model.side_effect = [
            {"model_name": "LinearRegression", "run_id": "lr_run", "val_rmse": 0.5, "test_rmse": 0.6},
            {"model_name": "RandomForest", "run_id": "rf_run", "val_rmse": 0.4, "test_rmse": 0.5},
            {"model_name": "XGBoost", "run_id": "xgb_run", "val_rmse": 0.3, "test_rmse": 0.4}
        ]
    ...
    @patch('flows.model_training.split_data')
    @patch('flows.model_training.load_processed_data_from_s3')
    def test_train_models_data_loading_error(self, mock_load_data, mock_split_data, mock_train_model):
    ...
    @patch('flows.model_training.split_data')
    @patch('flows.model_training.load_processed_data_from_s3')
    def test_train_models_split_error(self, mock_load_data, mock_split_data, mock_train_model, sample_complete_data):
...
@pytest.mark.parametrize("test_size,expected_test_ratio", [
    (0.1, 0.1),
    (0.2, 0.2),
    (0.3, 0.3),
])
...
@pytest.mark.parametrize("target_column", [
    'arithmetic_mean',
    'first_max_value',
    'aqi'
])
